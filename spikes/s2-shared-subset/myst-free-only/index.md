...
A minimal corpus exercising every directive and role in the free/paid shared
subset documented in `spec-kit-hypotheses.md`.
...
When the driver presses the start button, the vehicle enters the running
state within 500ms.
```
...
When the driver presses the stop button, the vehicle leaves the running
state within 500ms.
```
...
The ignition controller emits a `START` signal on the CAN bus when the
...
The ignition controller emits a `STOP` signal on the CAN bus when the
